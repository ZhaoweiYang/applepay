...
- 颜色深浅是**顺序色阶**，表示 iOS 在当地移动系统中的份额；浅色/深色模式各自取了一套
  与背景对比达标的色阶。
- **缩放与平移**：`⌘`/`Ctrl` ＋滚轮、触控板双指捏合缩放（1×–14×），按住拖动平移，右上角有 ＋ / － / 重置按钮，
  键盘 `+` `-` `0` 和方向键同样可用。裸滚轮留给页面滚动（否则光标停在地图上就翻不动页），
  首次滚动会浮出一句提示。放大后小国的名字会自动浮现，而文字在屏幕上始终保持同样大小，
  尺寸图例也会跟着缩放挑选合适的参考值。
- **默认只画支持 Apple Pay 的市场**：不支持的 32 个国家/地区不会出现在图上（数据仍保留在完整数据表里）。
  想看全部，把工具栏 Apple Pay 那一组切到「全部」。
- **手动排除名单**：地图上方有一行"不展示的国家/地区"，默认排除 **中国大陆、美国、巴西、英国**
  这四个体量最大的市场（中美两家就占收录用户的四成，留着会把其余市场压成小方块）。
  双击地图上任意方块可以把它移出地图，点 ✕ 放回来，「全部恢复」一次清空。默认视图 = 57 − 4 = 53 个市场。
- **排行榜跟着筛选走**：Top 榜只统计当前画在图上的市场，标题、条数和占比都会跟着变
  （例如默认视图下第一名是日本，切到「不支持 Apple Pay」则是俄罗斯）。
- **筛选即重排**：按是否支持 Apple Pay、按风控档位、按当地主要语言（46 种）、按区域、按名称搜索，可任意组合。
  被筛掉的方块直接从图上移走，剩下的会重新做一次布局并等比放大铺满画布——所以每次筛选看到的都是
  "这批市场自己的世界地图"。统计行实时给出当前显示了多少市场、合计多少用户、占收录总量的比例
  （例如"支持 Apple Pay 的 57 个市场 = 82.8% 的 iPhone 用户"）。
- **风控筛选**：「排除虚拟卡高发」「排除高争议率」两个开关，外加一键组合
  **「风控友好市场」**（= 支持 Apple Pay ＋ 虚拟卡不高发 ＋ 争议率不高，46 个市场、约 72% 的 iPhone 用户）。
  ⚠️ 这两列没有权威公开统计（卡组织不公布逐国拒付率），是按 `data.js` 注释里那套规则做的
  **启发式三档分级**，用来快速圈定范围，不能作为风控决策的唯一依据；按自己的口径改 `vc` / `dr` 即可。
- 切换面积口径（iPhone 用户 / 智能手机用户 / 总人口，带补间动画）、悬停或点击查看明细、
  Top 20 排行、完整数据表。
...
- iOS 份额：StatCounter（gs.statcounter.com）2025 年前后的移动操作系统份额
- 主要语言：当地使用人数最多的一种（多语言国家只取一种，仅用于筛选）
- 虚拟卡发达程度 / 争议（拒付）率：启发式三档分级，评级规则与依据写在 `data.js` 注释里
- Apple Pay 是否支持：以 Apple 官方[《可以使用 Apple Pay 的国家和地区》](https://support.apple.com/en-us/102775)
  为准的 2025 年快照，另与 Apple 开发者站[按国家列出的支付服务商页面](https://developer.apple.com/apple-pay/payment-platforms/)
  交叉核对（89 条中 75 条一致，差异原因写在 `data.js` 的注释里）
...
├── iphone-map/
│   ├── index.html                                    # iPhone 用户量方块世界地图（页面 + 布局 + 缩放 + 筛选）
│   └── data.js                                       # 89 个国家/地区：人口 / 普及率 / iOS 份额 / 语言 / Apple Pay / 风控分级
