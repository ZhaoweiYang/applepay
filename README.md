...
- 颜色深浅是**顺序色阶**，表示 iOS 在当地移动系统中的份额；浅色/深色模式各自取了一套
  与背景对比达标的色阶。
- **缩放与平移**：滚轮 / 双指捏合缩放（1×–14×），按住拖动平移，右上角有 ＋ / － / 重置按钮，
  键盘 `+` `-` `0` 和方向键同样可用。放大后小国的名字会自动浮现，而文字在屏幕上始终保持同样大小，
  尺寸图例也会跟着缩放挑选合适的参考值。
- **默认只画支持 Apple Pay 的市场**：不支持的 32 个国家/地区不会出现在图上（数据仍保留在完整数据表里）。
  想看全部，把工具栏 Apple Pay 那一组切到「全部」。
- **手动排除名单**：地图上方有一行"不展示的国家/地区"，默认排除 **中国大陆、美国、加拿大、巴西、英国**
  这五个体量最大的市场（中美两家就占收录用户的四成，留着会把其余市场压成小方块）。
  双击地图上任意方块可以把它移出地图，点 ✕ 放回来，「全部恢复」一次清空。默认视图 = 57 − 5 = 52 个市场。
