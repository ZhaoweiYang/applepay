...
- **缩放与平移**：滚轮 / 双指捏合缩放（1×–14×），按住拖动平移，右上角有 ＋ / － / 重置按钮，
  ...
  尺寸图例也会跟着缩放挑选合适的参考值。
- **筛选**：按是否支持 Apple Pay、按风控档位、按当地主要语言（46 种）、按区域、按名称搜索，可任意组合。
  被筛掉的国家是"变暗"而不是消失，方便看清选中的那批在世界上怎么分布；统计行实时给出
  筛选结果占全部收录量的比例（例如"支持 Apple Pay 的 57 个市场 = 82.8% 的 iPhone 用户"）。
